...
const ChatThreadScreen: React.FC = () => {
  ...
  const correspondent: ChatCorrespondent | null =
    route.params?.correspondent ?? null;
  ...
  const [roomStatus, setRoomStatus] = useState<ChatRoomStatus>(
    route.params?.roomStatus ?? 'active',
  );
  const [input, setInput] = useState('');
  const loadingMoreRef = useRef(false);

  // Recebe mensagens em tempo real e reconcilia o estado otimista por uuid
  ...
  // Paginação: carrega mensagens mais antigas ao chegar no fim da lista invertida
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      {/* Header: voltar + avatar + nome do correspondente */}
