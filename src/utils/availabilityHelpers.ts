...
export const generateAvailableDates = (availability: ProfessionalAvailability[]): { date: string; times: string[] }[] => {
  ...
  availability.forEach(avail => {
    ...
    const times = Array.from({ length: end - start }, (_, i) => `${start + i}:00`);
    ...
    if (avail.recurrence_pattern === 'daily') {
      ...
      const dayNumbers = days.map(day => ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].indexOf(day));
    ...
    else if (avail.recurrence_pattern === 'monthly' && avail.start_day_of_month && avail.end_day_of_month) {
      const currentMonth = now.getMonth();
      const currentYear = now.getFullYear();
      
      for (let day = avail.start_day_of_month; day <= avail.end_day_of_month; day++) {
    ...
    else if (avail.start_day && (!avail.end_day || avail.start_day === avail.end_day)) {
      result.push({
