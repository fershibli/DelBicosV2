export const getIconForSubCategory = (title: string) => {
  ...
  if (
    lower.includes('estetic') ||
    lower.includes('massagem') ||
    lower.includes('spa')
  )
    return 'spa';
  if (lower.includes('manicure') || lower.includes('pedicure'))
  ...
  if (
    ...
    return 'dumbbell';
  if (lower.includes('enferm') || lower.includes('cuidador'))
    return 'user-nurse';
  if (lower.includes('fono')) return 'headset';
  if (lower.includes('médic') || lower.includes('consult'))
    return 'stethoscope';
